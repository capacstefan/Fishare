...
class PeerProgressRow(QWidget):
    """One row of progress per receiver."""
    ...
    def __init__(self, peer_name: str) -> None:
        ...
        self.label = QLabel(f"{peer_name} — queued")
    ...
    def update_progress(self, filename: str, done: int, total: int, bps: float, eta: str) -> None:
        pct = int(done * 100 / total) if total else 0
        self.bar.setValue(pct)
        self.label.setText(
            f"{self.peer_name} — {filename}: {pct}%  {_fmt_speed(bps)}  ETA {eta}"
        )

    def set_status(self, status: str) -> None:
        self.label.setText(f"{self.peer_name} — {status}")
...
class TransferTab(QWidget):
    ...
    def __init__(self, settings: dict, parent=None) -> None:
        ...
        self._rows: dict[str, PeerProgressRow] = {}
        ...
        self.discovered_list.setMinimumHeight(180)
        ...
        # Cap the transfers card so peer lists get priority for vertical space.
        progress_card.setMaximumHeight(180)
    ...
    def _on_send(self) -> None:
        ...
        for name in peers:
            if name not in self._rows:
                row = PeerProgressRow(name)
                self._progress_layout.insertWidget(self._progress_layout.count() - 1, row)
                self._rows[name] = row
        self.send_requested.emit(peers, files, pin)

    # ---------- slots from network ----------
    def on_task_progress(self, peer: str, filename: str, done: int, total: int, bps: float, eta: str) -> None:
        row = self._rows.get(peer)
        if row:
            row.update_progress(filename, done, total, bps, eta)
