...
A single Wire instance owns one TLS-wrapped socket. Every byte on the wire
belongs to a frame, eliminating the JSON-line + raw-bytes alignment hazards
of the previous protocol (no _drain, no resync polling, no timeout loops).
"""
...
def server_ctx() -> ssl.SSLContext:
    from . import crypto_utils  # local import to avoid cycle at module load
...
def open_offer(addr: str, port: int, offer: dict, accept_timeout: float = 180.0) -> Wire:
    """Open Wire, send offer, wait for accept. Returns the Wire on accept.

    Raises RuntimeError(reason) on rejection, ConnectionError on transport fail.
    """
