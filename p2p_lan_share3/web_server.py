"""Flask QR web server: phones upload files / send text to this device."""
...
<style>
  :root{--bg:#f4f5f7;--surface:#fff;--border:#e4e6eb;--text:#1d1d1f;
        --muted:#6e6e73;--accent:#0a84ff;--accent-hi:#409cff;
        --ok:#1f8a3b;--err:#c0382b;}
  *{box-sizing:border-box}
  html,body{margin:0;padding:0;background:var(--bg);color:var(--text);}
  body{font-family:-apple-system,"Segoe UI Variable","Segoe UI",Roboto,sans-serif;
       min-height:100vh;display:flex;flex-direction:column;align-items:center;
       padding:28px 18px 40px;}
  .wrap{width:100%;max-width:520px;display:flex;flex-direction:column;}
  h1{font-size:26px;font-weight:700;margin:4px 0 20px;text-align:center;letter-spacing:-0.01em;}
  h1 .sub{display:block;font-size:14px;font-weight:500;color:var(--muted);margin-top:4px;}
  .card{background:var(--surface);border:1px solid var(--border);border-radius:14px;
        padding:22px 20px;margin-bottom:16px;box-shadow:0 1px 2px rgba(0,0,0,0.03);}
  .card h2{font-size:17px;font-weight:600;margin:0 0 14px;text-align:center;}
  label.file{display:block;text-align:center;cursor:pointer;padding:14px;
             border:1px dashed var(--border);border-radius:10px;color:var(--muted);
             font-size:15px;background:#fafbfc;margin-bottom:14px;}
  label.file:hover{border-color:var(--accent);color:var(--accent);}
  input[type=file]{display:none;}
  #file-names{font-size:13px;color:var(--muted);text-align:center;
              margin:-8px 0 14px;min-height:18px;word-break:break-all;}
  textarea{width:100%;min-height:130px;resize:vertical;border:1px solid var(--border);
  ...
  button{width:100%;background:var(--accent);color:#fff;border:0;padding:13px 18px;
         border-radius:10px;font-size:16px;font-weight:600;cursor:pointer;
         transition:background .15s ease;}
  button:hover{background:var(--accent-hi);}
  .msg{text-align:center;font-size:14px;padding:10px 14px;border-radius:10px;margin-bottom:8px;}
...
class QrWebServer(QObject):
    ...
    stopped = pyqtSignal()
    file_received = pyqtSignal(str, str, int)  # name, saved_path, size
    ...
    def _make_app(self) -> Flask:
        ...
        def upload(tok):
            ...
            for fs in files:
                ...
                saved += 1
            return render_template_string(PAGE, name=name, msg=f"Uploaded {saved} file(s).", cls="ok")
        ...
        def text(tok):
            ...
            self.text_received.emit("Phone", txt[: config.QUICK_TEXT_MAX_CHARS])
            return render_template_string(PAGE, name=name, msg="Text sent.", cls="ok")

        return app

    def _qr_pixmap(self, data: str) -> QPixmap:
