...
class FIshareApp(ctk.CTk):
    def __init__(self, state, advertiser):
        ...
        # --- Toggle simplu Available/Busy ---
        self.status_switch = ctk.CTkSwitch(
            top,
            text=self._status_text(self.app_state.status),
            command=self._on_toggle_status
        )
        # seteaza pozitia initiala
        if self.app_state.status == AppStatus.AVAILABLE:
            self.status_switch.select()  # ON = available
        else:
            self.status_switch.deselect()  # OFF = busy
    ...
    @staticmethod
    def _status_text(status: AppStatus) -> str:
        return "Available" if status == AppStatus.AVAILABLE else "Busy"

    # --------- Callbacks UI ---------

    def _on_toggle_status(self):
        value = AppStatus.AVAILABLE if bool(self.status_switch.get()) else AppStatus.BUSY
        self.app_state.set_status(value)
        # actualizeaza textul de langa switch pentru claritate
        self.status_switch.configure(text=self._status_text(value))
    ...
    def _refresh_lists(self):
        ...
        for dev_id, dev in sorted(self.app_state.devices.items()):
            ...
            # doar bulina + id + nume; fara textul starii
            self.devices_list.insert("end", f"{dev_id}  {dev.name}  {dot}\n")
        self.devices_list.configure(state="disabled")
