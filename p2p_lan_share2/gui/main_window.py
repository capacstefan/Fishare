...
class MainWindow(QMainWindow):
    ...
    def _record_history(self, direction: str, peer: str, kind: str,
                        size: int = 0, count: int = 0) -> None:
        entry = {
            "date": _now(),
            "size": size,
            "count": count,
    ...
    def _on_send_files(self, peer_names: list, file_paths: list, pin: str) -> None:
        ...
        if offline and sent:
            ...
            names = ", ".join(offline)
            self.notify(
                f"Peer{'s' if len(offline) > 1 else ''} ({names}) "
                f"{'are' if len(offline) > 1 else 'is'} offline"
            )
        elif sent:
    ...
    def _on_recv_completed(self, sender, filenames, total_bytes) -> None:
        self._record_history("Received", sender, "File", total_bytes, len(filenames))
    ...
    def _on_text_received(self, sender, text) -> None:
        ...
        self._record_history("Received", sender, "QuickText", 0, 0)
    ...
    def _on_web_file_received(self, filename: str, path: str, size: int) -> None:
        self._record_history("Received", "Phone (QR)", "File", size, 1)
